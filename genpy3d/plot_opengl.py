...
class Plot_z_of_xy:
    ...
    def __post_init__(self):
        self.extent_x = self.axes.start[0], self.axes.start[0] + self.axes.extent[0]
        self.extent_y = self.axes.start[1], self.axes.start[1] + self.axes.extent[1]
    ...
    def _plot_surface(self):
        ...
        step = (self.extent_x[1] - self.extent_x[0]) / (self.precision - 1)
        for x in np.linspace(self.extent_x[0], self.extent_x[1], self.precision):
            glBegin(GL_TRIANGLE_STRIP)
            for y in np.linspace(self.extent_y[0], self.extent_y[1], self.precision):
    ...
    def _plot_lines(self):
        if self.grid:
            ...
            for x in np.linspace(self.extent_x[0], self.extent_x[1], self.x_grid_count):
                points = []
                for y in np.linspace(self.extent_y[0], self.extent_y[1], 500):
                    z = self.plotfunc(x, y)
                    points.append(self.axes.transform_from_graph((x, y, z)))
                for i in range(len(points) - 1):
                    self._draw_cylinder(points[i], points[i + 1])

            for y in np.linspace(self.extent_y[0], self.extent_y[1], self.y_grid_count):
                points = []
                for x in np.linspace(self.extent_x[0], self.extent_x[1], 500):
                    z = self.plotfunc(x, y)
                    points.append(self.axes.transform_from_graph((x, y, z)))
                for i in range(len(points) - 1):
                    self._draw_cylinder(points[i], points[i + 1])
...
class Plot_xyz_of_uv:
    ...
    def with_grid(self, u_grid_count=20, v_grid_count=20, grid_color=(0.3, 0.3, 0.3), line_radius=0.004, grid_precision=500):
        self.grid = True
        self.u_grid_count = u_grid_count
        self.v_grid_count = v_grid_count
        self.grid_color = grid_color
        self.line_radius = line_radius
        self.grid_precision = grid_precision
        return self
    ...
    def _plot_surface(self):
        ...
        step = (self.extent_u[1] - self.extent_u[0]) / (self.precision - 1)
        for u in np.linspace(self.extent_u[0], self.extent_u[1], self.precision):
            glBegin(GL_TRIANGLE_STRIP)
            for v in np.linspace(self.extent_v[0], self.extent_v[1], self.precision):
                x1 = self.plotfunc_x(u, v)
                y1 = self.plotfunc_y(u, v)
                z1 = self.plotfunc_z(u, v)
                x2 = self.plotfunc_x(u + step, v)
                y2 = self.plotfunc_y(u + step, v)
                z2 = self.plotfunc_z(u + step, v)

                glColor3f(*self._get_color(self.fore_colormap, z1))
                glVertex3f(*self.axes.transform_from_graph((x1, y1, z1)))
                glColor3f(*self._get_color(self.fore_colormap, z2))
                glVertex3f(*self.axes.transform_from_graph((x2, y2, z2)))
            glEnd()
        ...
        for u in np.linspace(self.extent_u[0], self.extent_u[1], self.precision):
            ...
            for v in np.linspace(self.extent_v[0], self.extent_v[1], self.precision):
                ...
                glVertex3f(*self.axes.transform_from_graph((x, y, z1)))
                glColor3f(*self._get_color(self.fore_colormap, z2))
                glVertex3f(*self.axes.transform_from_graph((x, y, z2)))
            glEnd()
